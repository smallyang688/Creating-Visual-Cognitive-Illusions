...
def configure_pipeline(pipe, device):
    if device == "cuda":
...
stage_1 = configure_pipeline(stage_1, args.device)
...
if not args.skip_stage_2:
    ...
    stage_2 = configure_pipeline(stage_2, args.device)

# Make DeepFloyd IF stage III (which is just Stable Diffusion 4x Upsampler)
if args.generate_1024 and not args.skip_stage_2:
    stage_3 = DiffusionPipeline.from_pretrained(
                    "stabilityai/stable-diffusion-x4-upscaler", 
                    torch_dtype=torch.float16
                )
    stage_3 = configure_pipeline(stage_3, args.device)
